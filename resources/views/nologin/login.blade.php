  ...
  <script>
    ...
    function getErrorMessage(errorCode, field = '') {
      const errorMessages = {
        ...
        'ACCOUNT_LOCKED': 'Tài khoản tạm thời bị khóa do đăng nhập sai quá nhiều lần. Vui lòng thử lại sau.',
    ...
    async function doLogin() {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showToast('Email không hợp lệ', 'warning'); return; }
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (error === 'EMAIL_NOT_FOUND') {
            ...
            showToast(serverMessage || getErrorMessage('WRONG_PASSWORD'), 'error');
          } else if (error === 'ACCOUNT_BLOCKED') {
            showToast(serverMessage || getErrorMessage('ACCOUNT_BLOCKED'), 'error');
          } else if (error === 'ACCOUNT_LOCKED') {
            showToast(serverMessage || getErrorMessage('ACCOUNT_LOCKED'), 'warning');
          } else {
            showToast(serverMessage || getErrorMessage(error), 'error');
          }
