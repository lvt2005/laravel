        ...
        <div class="section-card full-width">
          ...
          <div class="profile-info">
            <div class="profile-avatar">
              <img src="{{ asset('frontend/img/logocanhan.jpg') }}" alt="Ảnh đại diện" id="profileImage">
              ...
              <div class="rating-summary" onclick="openReviewsModal()" style="cursor: pointer; margin-top: 10px; text-align: center;">
                ...
                <div style="font-size: 13px; color: #666; margin-top: 3px;">
                  <span id="doctorReviewCount">0</span> đánh giá
                </div>
              </div>
            </div>
              ...
              <div class="info-row">
                <div class="info-field" style="flex: 1;">
                  ...
                  <div id="servicesContainer" style="display: flex; flex-wrap: wrap; gap: 8px; min-height: 80px; padding: 12px; border: 2px solid #e8eaf6; border-radius: 10px; background: #f8f9fa;">
                    <span style="color: #999; font-size: 14px;">Chọn chuyên khoa trước để hiển thị dịch vụ</span>
                  </div>
        ...
        <div class="modal-body">
          <form id="newNoteForm">
            <div class="form-group" style="position: relative;">
              <label>Tên bệnh nhân * (từ lịch đang khám hoặc đã hoàn thành)</label>
            ...
            <div style="background: #e8f4fd; padding: 12px; border-radius: 8px; margin-top: 15px;">
              <p style="font-size: 13px; color: #0c5460; margin: 0;">
                ...
                Có thể tạo ghi chú cho các lịch hẹn <strong>đang diễn ra</strong> hoặc <strong>đã hoàn thành</strong>.
              </p>
            </div>
          </form>
        </div>
  ...
  <div id="reviewsModal" class="modal">
    <div class="modal-content" style="max-width: 600px;">
      <div class="modal-header" style="background: linear-gradient(135deg, #ffc107, #ff9800);">
        <h3><i class="ri-star-line"></i> Đánh giá từ bệnh nhân</h3>
      ...
      <div class="modal-body" style="max-height: 500px; overflow-y: auto; padding: 15px;">
        <div id="reviewsSummary" style="text-align: center; padding: 15px; background: #f8f9fa; border-radius: 10px; margin-bottom: 15px;">
          ...
          <div style="color: #666; margin-top: 5px;"><span id="modalReviewCount">0</span> đánh giá</div>
        </div>
  ...
  <script>
    ...
    async function saveProfileChanges() {
      ...
      try {
        ...
        // Update doctor profile (degree, experience, specialization, services)
        ...
        if (!doctorResponse.ok) {
          console.warn('Không thể cập nhật thông tin bác sĩ');
        }
        ...
        showToast('Có lỗi xảy ra khi lưu thông tin: ' + err.message, 'error');
      }
    }
      ...
      if (token) {
        try {
          await fetch('/api/profile/two-factor', {
            ...
            body: JSON.stringify({ two_factor_enabled: settings.twoFactorAuth })
          });
        } catch (e) {
    ...
    (function () {
      ...
      async function loadDoctorAppointments() {
        console.log('loadDoctorAppointments called');
        try {
          const token = localStorage.getItem('access_token');
          if (!token) {
          ...
          if (response.ok) {
            ...
            console.log('Loaded appointments:', doctorAppointments.length);
          } else {
      ...
      function renderDw() {
        console.log('renderDw called, appointments:', doctorAppointments.length);
              ...
              items.forEach(apt => {
                ...
                body += `
                  <div class="dw-card ${statusClass}" onclick="openAppointmentDetail(${JSON.stringify(apt).replace(/"/g, '&quot;')}, '${timeStatus.status}')">
                    <h4 style="font-weight: 700; font-size: 14px; margin-bottom: 8px;">${apt.patient_name || 'Bệnh nhân'}</h4>
                    <div class="dw-meta" style="font-size: 11px; line-height: 1.6;">
                      <div><i class="ri-hospital-line"></i> ${apt.clinic_name || 'Chưa xác định'}</div>
                      <div><i class="ri-time-line"></i> ${startTime} - ${endTime} (${duration} phút)</div>
      ...
      document.addEventListener('DOMContentLoaded', () => {
        console.log('Calendar IIFE DOMContentLoaded');
        ...
        const di = document.getElementById('dwDateInput');
        const table = document.getElementById('dwCalendarTable');
        console.log('Calendar elements:', { prev: !!prev, next: !!next, today: !!today, di: !!di, table: !!table });
    ...
    function openAppointmentDetail(aptData, timeStatus) {
      ...
      body.innerHTML = `
        <div class="detail-row">
          ...
          <div class="detail-value"><strong style="font-size: 16px;">${aptData.patient_name || 'Chưa có thông tin'}</strong></div>
        </div>
        <div class="detail-row">
        ...
        <div class="detail-row">
          ...
          <div class="detail-value">${aptData.patient_email || 'Chưa có'}</div>
        </div>
        <div class="detail-row">
        ...
        <div class="detail-row">
          ...
          <div class="detail-value">${aptData.specialization_name || 'Chưa xác định'}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Dịch vụ:</div>
          <div class="detail-value">${aptData.service_name || 'Chưa xác định'}${aptData.service_duration ? ' (' + aptData.service_duration + ' phút)' : ''}</div>
        </div>
        <div class="detail-row">
          <div class="detail-label">Yêu cầu bệnh nhân:</div>
          <div class="detail-value" style="font-style: ${aptData.notes ? 'normal' : 'italic'}; color: ${aptData.notes ? '#333' : '#999'};">${aptData.notes || 'Không có yêu cầu đặc biệt'}</div>
        </div>
        <div class="detail-row">
    ...
    async function openReviewsModal() {
      ...
      reviewsList.innerHTML = '<div style="text-align: center; color: #999; padding: 30px;">Đang tải đánh giá...</div>';
      ...
      try {
        ...
        if (currentDoctorId) {
          ...
          reviewsList.innerHTML = '<div style="text-align: center; color: #999; padding: 30px;">Không tìm thấy thông tin bác sĩ</div>';
        }
      } catch (err) {
        console.error('Error loading reviews:', err);
        reviewsList.innerHTML = '<div style="text-align: center; color: #999; padding: 30px;">Không thể tải đánh giá</div>';
      }
    }
    ...
    function renderReviewsList() {
      ...
      if (!doctorReviews || doctorReviews.length === 0) {
        reviewsList.innerHTML = '<div style="text-align: center; color: #999; padding: 30px;"><i class="ri-star-line" style="font-size: 48px; display: block; margin-bottom: 10px;"></i>Chưa có đánh giá nào</div>';
        return;
      }
      ...
      reviewsList.innerHTML = doctorReviews.map(review => `
        <div style="background: #fff; border: 1px solid #eee; border-radius: 10px; padding: 15px;">
          <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
            <div>
              <strong style="color: #333;">${review.patient_name || 'Bệnh nhân'}</strong>
          ...
          ${review.comment ? `<p style="color: #666; margin: 0; line-height: 1.5;">${review.comment}</p>` : '<p style="color: #999; font-style: italic; margin: 0;">Không có nhận xét</p>'}
        </div>
      `).join('');
    }
    ...
    async function loadServicesForSpecialty(specId) {
      ...
      if (!specId) {
        servicesContainer.innerHTML = '<span style="color: #999; font-size: 14px;">Chọn chuyên khoa trước để hiển thị dịch vụ</span>';
        return;
      }

      servicesContainer.innerHTML = '<span style="color: #999; font-size: 14px;"><i class="ri-loader-4-line" style="animation: spin 1s linear infinite;"></i> Đang tải...</span>';
          ...
          if (services && Array.isArray(services) && services.length > 0) {
            ...
            services.forEach(service => {
              ...
              checkbox.innerHTML = `
                ...
                <small style="color: #4a69bd; font-weight: 500;">${service.price ? parseInt(service.price).toLocaleString() + 'đ' : ''}</small>
              `;
            ...
            servicesContainer.innerHTML = '<span style="color: #999; font-size: 14px;">Chưa có dịch vụ cho chuyên khoa này</span>';
          }
        }
      } catch (error) {
        console.error('Error loading services:', error);
        servicesContainer.innerHTML = '<span style="color: #e74c3c; font-size: 14px;">Lỗi tải dịch vụ</span>';
      }
    }
    ...
    document.addEventListener('DOMContentLoaded', async function () {
      ...
      try {
        ...
        doctorName = userData.full_name || 'Bác sĩ';
        ...
        // Load doctor profile (specialization, degree, experience, services)
        const doctorResponse = await fetch('/api/profile/doctor', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (doctorResponse.ok) {
      ...
      if (savedProfile && !document.getElementById('profileEmail').value) {
        ...
        doctorName = profileData.name || 'Bác sĩ';
    ...
    function displayPatientSuggestions(patients) {
      ...
      if (patients.length === 0) {
        suggestionsContainer.innerHTML = '<div class="autocomplete-loading">Không tìm thấy bệnh nhân (đã hoàn thành hoặc đang khám)</div>';
        suggestionsContainer.classList.add('show');
        return;
      }
      ...
      suggestionsContainer.innerHTML = patients.map(p => {
        const statusBadge = p.status === 'IN_PROGRESS' 
          ? '<span class="status-badge in-progress">Đang khám</span>' 
