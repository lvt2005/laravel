...
(function () {
  ...
  function init() {
    ...
    loadProfileData().then(() => {
      // Kiểm tra xem có cần mở chi tiết lịch hẹn từ URL không
      checkOpenAppointmentFromUrl();
    });
  }
  
  // Kiểm tra và mở chi tiết lịch hẹn từ URL
  function checkOpenAppointmentFromUrl() {
    const urlParams = new URLSearchParams(window.location.search);
    const openAppointmentId = urlParams.get('open') || localStorage.getItem('openAppointmentId');
    const section = urlParams.get('section');
    
    // Chuyển đến section lịch hẹn nếu cần
    if (section === 'appointments') {
      const appointmentNav = document.querySelector('[data-section="appointments"]');
      if (appointmentNav) {
        appointmentNav.click();
      }
    }
    
    // Mở chi tiết lịch hẹn nếu có ID
    if (openAppointmentId) {
      localStorage.removeItem('openAppointmentId');
      // Tìm lịch hẹn trong state
      setTimeout(() => {
        const appointment = state.appointments.find(a => a.id == openAppointmentId);
        if (appointment) {
  ...
  function showAppointmentDetail(item) {
    ...
    if (paymentStatus === 'PAID' && (item.status === 'confirmed' || item.status === 'available')) {
      ...
      `;
    } else if ((paymentStatus === 'UNPAID' || paymentStatus === '') && (item.status === 'confirmed' || item.status === 'available')) {
      // Kiểm tra có thể hủy không (trước ngày hẹn 24h)
      const appointmentDate = new Date(item.appointment_date + 'T' + (item.start_time || '08:00:00'));
      const now = new Date();
      ...
      if (hoursUntilAppointment > 24) {
        actionButtons = `
          <div style="margin-top: 20px; text-align: center;">
            <button class="btn btn-primary btn-pay-appointment" data-id="${item.id}" data-amount="${item.fee_amount || 0}" style="margin-right: 10px;">
              <i class="fas fa-credit-card"></i> Thanh toán ngay
            </button>
            <button class="btn btn-danger btn-cancel-modal" data-id="${item.id}">
              <i class="fas fa-times"></i> Hủy lịch hẹn
            </button>
            <small style="display: block; margin-top: 8px; color: #888;">Có thể hủy lịch trước 24 giờ so với giờ hẹn</small>
          </div>
        `;
      } else {
        actionButtons = `
          <div style="margin-top: 20px; text-align: center;">
            <button class="btn btn-primary btn-pay-appointment" data-id="${item.id}" data-amount="${item.fee_amount || 0}">
              <i class="fas fa-credit-card"></i> Thanh toán ngay
            </button>
            <small style="display: block; margin-top: 8px; color: #888;"><i class="fas fa-info-circle"></i> Không thể hủy (đã quá hạn 24h trước giờ hẹn)</small>
          </div>
        `;
      }
    }
    ...
    const payBtn = modalBody.querySelector('.btn-pay-appointment');
    if (payBtn) {
    ...
    // Bind cancel button in modal
    const cancelBtn = modalBody.querySelector('.btn-cancel-modal');
    if (cancelBtn) {
      cancelBtn.addEventListener('click', async function() {
        const appointmentId = this.dataset.id;
        
        const confirmed = confirm(
          '⚠️ XÁC NHẬN HỦY LỊCH HẸN ⚠️\n\n' +
          'Bạn có chắc chắn muốn hủy lịch hẹn này?\n\n' +
          'Bạn có thể đặt lại lịch hẹn mới sau khi hủy.'
        );
        ...
        try {
          ...
          this.disabled = false;
          this.innerHTML = '<i class="fas fa-times"></i> Hủy lịch hẹn';
        }
      });
    }
  }
    ...
    document.querySelectorAll('.btn-confirm-appointment').forEach(btn => {
      btn.addEventListener('click', async function() {
        ...
        const confirmed = confirm(
          ...
          'Sau khi xác nhận, bạn có thể hủy lịch hẹn trong vòng 24 giờ trước giờ hẹn.\n\n' +
  ...
  function buildPaymentActions(item) {
    ...
    if (status === 'confirmed' || status === 'available') {
      ...
      // Chưa thanh toán - hiển thị nút thanh toán và hủy lịch
      // Kiểm tra có thể hủy không (trước ngày hẹn 24h)
      const appointmentDate = new Date(item.appointment_date + 'T' + (item.start_time || '08:00:00'));
      const now = new Date();
      const hoursUntilAppointment = (appointmentDate - now) / (1000 * 60 * 60);
      ...
      if (hoursUntilAppointment > 24) {
        return `
          ...
          <button class="btn btn-danger btn-cancel-confirmed-appointment" data-id="${item.id}" style="margin-left: 8px;">
        ...
        return `
          ...
          <span class="text-muted" style="margin-left: 8px; font-size: 12px;"><i class="fas fa-info-circle"></i> Không thể hủy (đã quá hạn 24h trước giờ hẹn)</span>
        `;
      }
    }
  ...
  function bindPaymentActions() {
    ...
    // Bind cancel confirmed appointment button (within 24h rule)
    document.querySelectorAll('.btn-cancel-confirmed-appointment').forEach(btn => {
      btn.addEventListener('click', async function(e) {
        ...
        const confirmed = confirm(
          '⚠️ XÁC NHẬN HỦY LỊCH HẸN ⚠️\n\n' +
          'Bạn có chắc chắn muốn hủy lịch hẹn này?\n\n' +
          'Lưu ý: Bạn có thể đặt lại lịch hẹn mới sau khi hủy.\n\n' +
          'Nhấn OK để tiếp tục hủy.'
        );
        ...
        try {
          ...
          if (resp.ok && data.success) {
            alert('✅ Đã hủy lịch hẹn thành công!\n\nBạn có thể đặt lịch hẹn mới nếu cần.');
            renderAppointments();
          } else {
            throw new Error(data.message || 'Không thể hủy lịch hẹn');
          }
  ...
  async function openRefundModal(appointmentId, amount) {
    ...
    let paymentMethods = state.dashboard?.payment_methods || [];
    
    // If no payment methods, show form to add one first
    if (!paymentMethods || paymentMethods.length === 0) {
      showAddPaymentBeforeRefund(appointmentId, amount);
      return;
    }
    
    // Show refund confirmation modal with payment method selection
    showRefundRequestForm(appointmentId, amount, paymentMethods);
  }
  
  // Show form to add payment method before refund
  function showAddPaymentBeforeRefund(appointmentId, amount) {
      ...
      <div style="padding: 20px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #ffc107, #ff9800); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px;">
            <i class="fas fa-credit-card" style="font-size: 36px; color: white;"></i>
          </div>
          <h4 style="margin: 0 0 10px 0;">Thêm phương thức thanh toán</h4>
          <p style="color: #666; margin: 0;">Bạn cần thêm phương thức thanh toán trước khi yêu cầu hoàn tiền</p>
        </div>
        
        <div style="background: #e3f2fd; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #2196f3;">
          <p style="margin: 0; color: #1565c0; font-size: 14px;">
            <i class="fas fa-info-circle"></i> Tiền hoàn trả sẽ được chuyển vào tài khoản bạn cung cấp bên dưới.
          </p>
        </div>
        
        <div class="form-group" style="margin-bottom: 15px;">
          <label style="display: block; margin-bottom: 5px; font-weight: 500;">Loại phương thức <span style="color: red;">*</span></label>
          <select id="refundPaymentType" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px;">
        ...
        <div class="form-group" style="margin-bottom: 15px;">
          <label style="display: block; margin-bottom: 5px; font-weight: 500;">Tên tài khoản/chủ thẻ <span style="color: red;">*</span></label>
          <input type="text" id="refundAccountName" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px;" placeholder="VD: NGUYEN VAN A">
        </div>
        
        <div class="form-group" style="margin-bottom: 15px;">
          <label style="display: block; margin-bottom: 5px; font-weight: 500;">Số tài khoản/số điện thoại <span style="color: red;">*</span></label>
          <input type="text" id="refundAccountNumber" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px;" placeholder="VD: 1234567890">
        </div>
        
        <div class="form-group" id="bankNameGroup" style="margin-bottom: 15px;">
          <label style="display: block; margin-bottom: 5px; font-weight: 500;">Tên ngân hàng <span style="color: red;">*</span></label>
          <input type="text" id="refundBankName" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px;" placeholder="VD: Vietcombank">
        </div>
        
        <button id="savePaymentAndContinueBtn" class="btn btn-primary" style="width: 100%; padding: 12px;" data-id="${appointmentId}" data-amount="${amount}">
    ...
    document.getElementById('refundPaymentType').addEventListener('change', function() {
    ...
    document.getElementById('savePaymentAndContinueBtn').addEventListener('click', async function() {
      const btn = this;
      const paymentType = document.getElementById('refundPaymentType').value;
      ...
      try {
        // Save payment method
        const resp = await window.AuthAPI.apiFetch('/profile/payment-methods', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            method_name: paymentType === 'bank' ? bankName : (paymentType === 'momo' ? 'MoMo' : 'ZaloPay'),
        ...
        if (resp.ok) {
          // Refresh dashboard to get new payment methods
          await refreshDashboard();
          // Now show the refund form
          const paymentMethods = state.dashboard?.payment_methods || [];
          showRefundRequestForm(btn.dataset.id, parseFloat(btn.dataset.amount), paymentMethods);
        } else {
          throw new Error(data.message || 'Không thể lưu phương thức thanh toán');
        }
      } catch (err) {
        alert('Lỗi: ' + (err.message || 'Không thể lưu phương thức thanh toán'));
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-save"></i> Lưu và tiếp tục';
      }
    });
  }
  
  // Show refund request form with all details
  function showRefundRequestForm(appointmentId, amount, paymentMethods) {
    let modal = byId('refundModal');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'refundModal';
    ...
    const paymentOptions = paymentMethods.map(pm => 
      `<option value="${pm.id}">${pm.method_name} - ${pm.masked_detail || pm.account_number || ''}</option>`
    ).join('');
    ...
    modalBody.innerHTML = `
      <div style="padding: 20px;">
        <div style="text-align: center; margin-bottom: 20px;">
          ...
          <p style="color: #666; margin: 0;">Số tiền hoàn: <strong style="color: #dc3545; font-size: 24px;">${formatCurrency(amount)}</strong></p>
        </div>
        
        <div style="background: #ffebee; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #f44336;">
          <p style="margin: 0; color: #c62828; font-size: 14px; font-weight: 500;">
            <i class="fas fa-exclamation-circle"></i> <strong>Cảnh báo:</strong> Sau khi yêu cầu hoàn tiền được duyệt, bạn sẽ không thể thanh toán lại cho lịch hẹn này.
          </p>
        </div>
        
        <div class="form-group" style="margin-bottom: 15px;">
          <label style="display: block; margin-bottom: 5px; font-weight: 500;">Lý do hoàn tiền <span style="color: red;">*</span></label>
          <select id="refundReasonSelect" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px; margin-bottom: 10px;">
            <option value="">-- Chọn lý do --</option>
            <option value="Không thể tham gia lịch hẹn">Không thể tham gia lịch hẹn</option>
            <option value="Đổi lịch khác phù hợp hơn">Đổi lịch khác phù hợp hơn</option>
            <option value="Sức khỏe đã cải thiện">Sức khỏe đã cải thiện</option>
            <option value="Chuyển đến bác sĩ khác">Chuyển đến bác sĩ khác</option>
            <option value="Lý do cá nhân">Lý do cá nhân</option>
            <option value="other">Khác (nhập lý do)</option>
          </select>
          <textarea id="refundReasonText" rows="2" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px; resize: none; display: none;" placeholder="Nhập lý do cụ thể..."></textarea>
        </div>
        
        <div class="form-group" style="margin-bottom: 15px;">
          <label style="display: block; margin-bottom: 5px; font-weight: 500;">Phương thức nhận hoàn tiền <span style="color: red;">*</span></label>
          <select id="refundPaymentMethod" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px;">
            ${paymentOptions}
          </select>
        </div>
    ...
    document.getElementById('refundReasonSelect').addEventListener('change', function() {
    ...
    document.getElementById('sendRefundOtpBtn').addEventListener('click', async function() {
      ...
      const reasonText = document.getElementById('refundReasonText').value.trim();
      const paymentMethodId = document.getElementById('refundPaymentMethod').value;
      ...
      if (reasonSelect === 'other' && !reasonText) {
        alert('Vui lòng nhập lý do cụ thể');
        return;
      }
      
      const reason = reasonSelect === 'other' ? reasonText : reasonSelect;
      ...
      try {
        ...
        if (resp.ok) {
          showRefundOtpInput(appointmentId, amount, reason, paymentMethodId);
        } else {
  ...
  function showRefundOtpInput(appointmentId, amount, reason, paymentMethodId) {
    const modalBody = document.getElementById('refundModalBody');
    ...
    modalBody.innerHTML = `
      <div style="padding: 20px; text-align: center;">
        ...
        <button id="confirmRefundOtpBtn" class="btn btn-danger" style="width: 100%; padding: 12px;" data-id="${appointmentId}" data-reason="${escapeHtml(reason)}" data-payment="${paymentMethodId}">
    ...
    document.getElementById('confirmRefundOtpBtn').addEventListener('click', async function() {
      ...
      const paymentMethodId = btn.dataset.payment;
      ...
      try {
        const resp = await window.AuthAPI.apiFetch(`/profile/appointments/${appointmentId}/refund/confirm`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ otp, reason, payment_method_id: paymentMethodId })
        });
        ...
        alert('Lỗi: ' + (err.message || 'Mã xác nhận không đúng'));
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-check"></i> Xác nhận hoàn tiền';
        // Clear OTP input for retry
        document.getElementById('refundOtpInput').value = '';
        document.getElementById('refundOtpInput').focus();
      }
    });
  ...
  async function processPayment(appointmentId, methodId, amount) {
    ...
    try {
      const settingsResp = await fetch('/api/public/system-settings');
      const settingsData = await settingsResp.json();
      
      if (settingsResp.ok && settingsData.success && !settingsData.data.payment_enabled) {
        closeModal('paymentSelectModal');
        alert('⚠️ Chức năng thanh toán đang được bảo trì. Vui lòng thử lại sau hoặc liên hệ hỗ trợ.');
        return;
      }
    } catch (e) {
      console.warn('Could not check payment settings:', e);
      // Continue with payment if settings check fails
    }
    ...
    if (!overlay) {
      ...
      overlay.innerHTML = `
        <div style="background: white; padding: 40px 60px; border-radius: 20px; text-align: center;">
          ...
          <h3 style="margin: 0; color: #28a745;">Thanh toán thành công!</h3>
          <p style="margin: 10px 0 0; color: #666;">Cảm ơn bạn đã thanh toán</p>
        </div>
      `;
      document.body.appendChild(overlay);
    }
  ...
  function navigateToNotificationTarget(notif) {
      ...
      switch (notif.type) {
        case 1: // Lịch hẹn (Appointment)
          if (relatedId) {
            // Switch to appointments section first
            const appointmentsNav = document.querySelector('.nav-item[data-section="appointments"]');
            if (appointmentsNav) appointmentsNav.click();
            
            // Open the appointment detail after a short delay
            setTimeout(() => {
              const appointment = state.appointments.find(a => a.id == relatedId);
              if (appointment) {
                showAppointmentDetail(appointment);
              }
            }, 300);
            ...
            const appointmentsNav2 = document.querySelector('.nav-item[data-section="appointments"]');
            if (appointmentsNav2) appointmentsNav2.click();
          }
          break;
          
        case 2: // Đánh giá (Review)
          // Switch to appointments section where reviews can be viewed
          const appointmentsNav3 = document.querySelector('.nav-item[data-section="appointments"]');
          if (appointmentsNav3) appointmentsNav3.click();
          break;
          
        case 3: // Forum
          if (relatedId) {
            // Navigate to forum section within profile
            ...
            if (forumNav) {
              ...
              setTimeout(() => {
                if (typeof openForumPost === 'function') {
                  openForumPost(relatedId);
                } else if (typeof viewForumPost === 'function') {
                  viewForumPost(relatedId);
                }
              }, 500);
            }
          }
  ...
  window.openRefundModal = openRefundModal; // Uses the full openRefundModal function defined earlier
