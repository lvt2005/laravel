...
// Dynamic API base: Use relative path for same-origin requests (Docker port 80 or Laravel port 8000)
// Falls back to localhost:8000 only for file:// protocol or other non-standard access
(function(){
...
function setTokens(data){
  ...
  console.log('[auth.js] setTokens:', {accessToken, refreshToken, sessionId});
}
...
(function(){
  ...
  refreshToken = storage.get('refresh_token');
  sessionId = storage.get('session_id');
  console.log('[auth.js] restore tokens from storage:', {accessToken, refreshToken, sessionId, origin: location.origin});
})();
