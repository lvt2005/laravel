...
class MailService
{
    ...
    /**
     * Check if email is enabled for user type
     * @param string $userType 'USER', 'DOCTOR', 'ADMIN' or null for general
     */
    private function isEmailEnabledForType(?string $userType = null): bool
    {
        // First check global email setting
        if (!$this->isEmailEnabled()) {
            \Log::info('Email sending disabled globally');
            return false;
        }
        
        // If no specific user type, allow (e.g., for booking verification)
        if (!$userType) {
            return true;
        }
        ...
        switch (strtoupper($userType)) {
            ...
            case 'ADMIN':
                // Admin emails are always enabled when global email is enabled
                return true;
            default:
                return true;
        }
     ...
     * @param string $userType 'USER' or 'DOCTOR' to check email settings
     */
    public function sendAppointmentReminder($toEmail, $toName, $appointmentDate, $appointmentTime, $doctorName, $clinicName, $userType = 'USER')
    {
        ...
        if (!$this->isEmailEnabledForType($userType)) {
            \Log::info("Email sending disabled for {$userType}. Appointment reminder not sent to: {$toEmail}");
            return ['success' => false, 'message' => 'Email sending is disabled for this user type'];
        }
     ...
     * @param string $userType 'USER' to check email settings
     */
    public function sendPaymentConfirmation($toEmail, $toName, $transactionId, $amount, $paymentMethod, $appointmentDetails, $userType = 'USER')
    {
        // Check if email is enabled for this user type
        if (!$this->isEmailEnabledForType($userType)) {
            \Log::info("Email sending disabled for {$userType}. Payment confirmation not sent to: {$toEmail}");
            return ['success' => false, 'message' => 'Email sending is disabled for this user type'];
        }
     ...
     * @param string|null $userType User type for email filtering (USER, DOCTOR, ADMIN)
     */
    public function send($toEmail, $toName, $subject, $htmlBody, $textBody = null, $userType = null)
    {
        // Check if email is enabled for this user type
        if (!$this->isEmailEnabledForType($userType)) {
            \Log::info("Email sending disabled for user type: {$userType}. Email not sent to: {$toEmail}");
            return ['success' => false, 'message' => 'Email sending is disabled for this user type'];
        }
    ...
    /**
     * Send email to user (checks USER email settings)
     */
    public function sendToUser($toEmail, $toName, $subject, $htmlBody, $textBody = null)
    {
        return $this->send($toEmail, $toName, $subject, $htmlBody, $textBody, 'USER');
    }
    
    /**
     * Send email to doctor (checks DOCTOR email settings)
     */
    public function sendToDoctor($toEmail, $toName, $subject, $htmlBody, $textBody = null)
    {
        return $this->send($toEmail, $toName, $subject, $htmlBody, $textBody, 'DOCTOR');
    }
