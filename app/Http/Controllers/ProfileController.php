...
class ProfileController extends Controller
{
    ...
    public function requestOnlinePayment(Request $request, $appointmentId)
    {
        ...
        // Set to PAID directly - no admin approval needed
        ...
        Notification::create([
            ...
            'message' => "Thanh toán của bạn cho lịch hẹn ngày {$appointment->appointment_date} đã thành công. Mã giao dịch: {$transactionId}",
            'type' => 2,
        ...
        // Send email confirmation - successful payment
        if ($user->email) {
            try {
        ...
        return response()->json(['success' => true, 'message'=>'Thanh toán thành công!', 'transaction_id' => $transactionId]);
    }
    ...
    public function getRefunds(Request $request)
    {
        ...
        $query = DB::table('appointment_schedules as a')
            ...
            ->join('user as du', 'd.user_id', '=', 'du.id')
            ->leftJoin('clinic as c', 'a.clinic_id', '=', 'c.id')
            ...
            ->select([
                ...
                'pm.method as refund_method_type',
                DB::raw("COALESCE(pm.wallet_number, pm.bank_account, pm.card_number) as refund_account")
            ]);
    ...
    public function getDoctorConfirmedAppointments(Request $request)
    {
        ...
        $appointments = DB::table('appointment_schedules as appt')
            ...
            ->select(
                ...
                'patient.avatar_url as patient_avatar',
    ...
    {
        try {
            $doctor = Doctor::with(['user', 'specialization', 'clinic', 'services'])->find($id);
            ...
            Log::error('Error fetching doctor', ['error' => $e->getMessage()]);
            return response()->json(['message' => 'Lỗi khi lấy thông tin bác sĩ'], 500);
        }
    }
    ...
    public function createDoctor(Request $request)
    {
        try {
            $validatedData = $request->validate([
                ...
                'service_ids.*' => 'integer|exists:treatment_service,id',
            ]);
    ...
    {
        try {
            $filter = $request->query('filter', 'all'); // pending, refund, refund_pending, success, failed, all

            // Base query with all necessary joins
            $baseQuery = function() {
                return \DB::table('appointment_schedules as a')
                    ...
                    ->leftJoin('payment_methods as rpm', 'a.refund_method_id', '=', 'rpm.id')
                    ->select(
                        'a.id',
                        'a.patient_id',
                        'a.patient_name',
                        'a.patient_phone',
                        \DB::raw('COALESCE(a.patient_email, u.email) as patient_email'),
                        'a.doctor_id',
                        'du.full_name as doctor_name',
                        'a.clinic_name',
                        'a.appointment_date',
                        ...
                        'a.refund_requested_at',
                        'a.refund_locked',
                        'rpm.method_name as refund_payment_method',
                        \DB::raw("COALESCE(rpm.wallet_number, rpm.bank_account, rpm.card_number) as refund_account"),
                        'a.created_at',
                        'a.updated_at'
                    );
            };
            ...
            switch ($filter) {
                ...
                case 'refund_pending':
                    // Refund requests
                    $query = $baseQuery()->where('a.refund_status', 'REQUESTED');
                    break;
                case 'refunded':
                case 'success':
                ...
                case 'failed':
                    ...
                    $query = $baseQuery()->where('a.refund_status', 'REJECTED');
                    break;
                default:
                    // All
                    $query = $baseQuery()->where(function($q) {
                        $q->whereIn('a.payment_status', ['PAID', 'REFUND_PENDING', 'REFUNDED'])
                          ->orWhere(function($q2) {
                              $q2->where('a.payment_status', 'UNPAID')
    ...
    /**
     * Check if a patient already has an appointment at the same time slot
     */
    public function checkPatientAppointment(Request $request)
    {
        try {
            $patientEmail = $request->query('patient_email');
            $patientPhone = $request->query('patient_phone');
            $appointmentDate = $request->query('appointment_date');
            $timeSlot = $request->query('time_slot');

            if ((!$patientEmail && !$patientPhone) || !$appointmentDate || !$timeSlot) {
                return response()->json([
            ...
            // Kiểm tra lịch hẹn của bệnh nhân qua email hoặc phone
            // Chỉ kiểm tra các trạng thái: pending, confirmed, booked
            $query = \DB::table('appointment_schedules')
                ->leftJoin('doctor', 'doctor.id', '=', 'appointment_schedules.doctor_id')
                ->leftJoin('user as doctor_user', 'doctor_user.id', '=', 'doctor.user_id')
                ->leftJoin('user as patient_user', 'patient_user.id', '=', 'appointment_schedules.patient_id')
                ->where('appointment_schedules.appointment_date', $appointmentDate)
                ->where('appointment_schedules.time_slot', $timeSlot)
                ->whereIn('appointment_schedules.status', ['pending', 'confirmed', 'booked'])
                ->select(
                    'appointment_schedules.id',
            ...
            if ($patientEmail) {
                $query->where(function($q) use ($patientEmail) {
                    $q->where('appointment_schedules.patient_email', $patientEmail)
                      ->orWhere('patient_user.email', $patientEmail);
                });
            } elseif ($patientPhone) {
                $query->where(function($q) use ($patientPhone) {
                    $q->where('appointment_schedules.patient_phone', $patientPhone)
                      ->orWhere('patient_user.phone', $patientPhone);
                });
            }
            ...
            if ($existing) {
                $doctorName = $existing->doctor_degree 
                    ? $existing->doctor_degree . ' ' . $existing->doctor_name 
                ...
                return response()->json([
                    ...
                    'message' => "Bạn đã có lịch hẹn với {$doctorName} vào thời gian này!"
                ]);
            }

            return response()->json([
                'has_appointment' => false,
                'message' => 'Chưa có lịch hẹn'
            ]);
        } catch (\Exception $e) {
            Log::error('Error checking patient appointment', ['error' => $e->getMessage()]);
            return response()->json([
                'has_appointment' => false,
                'message' => 'Không thể kiểm tra'
            ]);
    ...
    public function updateService(Request $request, $id)
    {
        try {
            ...
            \DB::table('treatment_service')->where('id', $id)->update($updateData);
