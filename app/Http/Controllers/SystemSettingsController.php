...
class SystemSettingsController extends Controller
{
    ...
    /**
     * Get forum reports (admin only)
     */
    public function getForumReports(Request $request): JsonResponse
    {
        try {
            $limit = $request->input('limit', 50);
            
            $reports = DB::table('forum_report')
                ->leftJoin('user', 'forum_report.user_id', '=', 'user.id')
                ->leftJoin('forum_post', 'forum_report.post_id', '=', 'forum_post.id')
                ->select([
                    'forum_report.*',
                    'user.full_name as reporter_name',
                ...
                ->orderByDesc('forum_report.created_at')
                ->limit($limit)
                ->get();
