...
class PublicController extends Controller
{
    ...
    public function getDoctors(Request $request)
    {
        ...
        $query = Doctor::with(['user', 'specialization', 'clinic'])
            ->where(function($q) {
                // Include ACTIVE and NULL (not explicitly INACTIVE)
                $q->where('doctor_status', 'ACTIVE')
                  ->orWhereNull('doctor_status');
            })
    ...
    public function getDoctorReviews($id)
    {
        ...
        if (!$doctor) {
            return response()->json(['success' => false, 'error' => 'DOCTOR_NOT_FOUND'], 404);
        }
    ...
    public function getServices(Request $request)
    {
        ...
        $services = $query->orderBy('name')->get(['id', 'name', 'description', 'price', 'duration_minutes', 'specialization_id', 'avatar_url', 'benefit1', 'benefit2', 'benefit3', 'benefit4']);
        ...
        return response()->json([
            ...
            'data' => $services->map(function ($service) {
                return [
                    ...
                    'benefit3' => $service->benefit3,
                    'benefit4' => $service->benefit4,
                ];
            })
        ]);
    }
    ...
    {
        $service = TreatmentService::where('is_active', true)->find($id);
    ...
    public function getDoctorWorkSchedule($id)
    {
        ...
        try {
            ...
            $sorted = $workSchedules->sortBy(function($schedule) use ($dayOrder) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error loading work schedule',
                'data' => []
            ]);
        }
    }
    ...
    /**
     * Get public system settings (for frontend feature toggling)
     */
    public function getPublicSystemSettings()
    {
        return response()->json([
            'success' => true,
            'data' => [
                ...
                'access_user_enabled' => \App\Models\SystemSetting::isUserAccessEnabled(),
                'access_doctor_enabled' => \App\Models\SystemSetting::isDoctorAccessEnabled(),
